...
TfLiteStatus EvalInt8DirectFilter(TfLiteContext* context, TfLiteNode* node) {
  const TfLiteEvalTensor* input =
      tflite::micro::GetEvalInput(context, node, kFullyConnectedInputTensor);
  const TfLiteEvalTensor* filter =
      tflite::micro::GetEvalInput(context, node, kFullyConnectedWeightsTensor);
  ...
  // Project2 model uses int8 FC weights, so skip the generic int4-unpack
  // wrapper and the generic Eval type switch. This keeps the same FC math and
  // CMSIS-NN arm_fully_connected_s8 backend.
